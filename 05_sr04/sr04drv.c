...
static int sr04_probe(struct platform_device *pdev)
{
	...
	/*申请中断并设置为双边沿触发*/
	err = request_irq(irq, sr04_isr, IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING, "r5ett_sr04", NULL);
	if (err != 0) {
		printk("request_irq is err\n");
	}
	
	/*2.创建设备节点*/	
	device_create(sr04_class, NULL, MKDEV(major, 0), NULL, "r5ett_sr04");
...
static int __init sr04_init(void)
{
	...
	printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
	/*确定主设备号*/
	major=register_chrdev(major, "r5ett_sr04", &sr04_ops);
	/*创建类*/
	sr04_class=class_create(THIS_MODULE, "r5ett_sr04");
	if (IS_ERR(sr04_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "r5ett_sr04");
